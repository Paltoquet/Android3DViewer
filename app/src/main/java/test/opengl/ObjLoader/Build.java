...
//        log.log(INFO,"Parsed face=" + face);
    ...
    // @TODO: I think I need to just delete this... because we now parse material lib files in Parse.java in sMaterialLib()
...
//    }
    ...
    public float[] getVertices(){
        float[] vertices = new float[verticesG.size()*3];
        for(int i=0; i<verticesG.size(); i++){
            VertexGeometric v = verticesG.get(i);
            vertices[i*3+0] = v.x;
            vertices[i*3+1] = v.y;
            vertices[i*3+2] = v.z;
        }
    ...
    public short[] getIndices(){
        ...
        for(int i=0; i<faces.size() ; i++){
            ArrayList<FaceVertex> vertices = faces.get(i).vertices;
            indices[i*3+0] = (short)vertices.get(0).index;
            indices[i*3+1] = (short)vertices.get(1).index;
            indices[i*3+2] = (short)vertices.get(2).index;
        }
        return indices;
    }
